...
void ReadPara(void)
{
    ...
    Eeprom_ReadByte(RUN_ADDR_BASE, &ParaInitFlag, 1);
    
    if(ParaInitFlag != User_Default_Param[0])
    {
        Eeprom_WriteByte(RUN_ADDR_BASE, User_Default_Param, USER_DEFAULT_LEN);
    }
    
    Eeprom_ReadByte(RUN_ADDR_BASE, Cur_Param, USER_DEFAULT_LEN);
