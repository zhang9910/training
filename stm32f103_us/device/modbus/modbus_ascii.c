...
extern uint8_t USART_RX_BUF[USART_MAX_DATALEN];//接收缓冲,最大USART_MAX_DATALEN个字节.
...
void MODBUS_ASCII_HandlRevData(uint8_t cyRevData)
{
    ...
    switch(g_cyRevState)
    {
        case ASCII_HEAD_STATE:    //  0
            ...
            USART_RX_BUF[g_cyRevBufffLen] = cyRevData;
            g_cyRevBufffLen++;
...
uint8_t MODBUS_ASCII_RecvData(uint8_t* cyRecvBuff, uint16_t *pCyLen)
{
    ...
    if (0 == MODBUS_ASCII_CheckAscii(&USART_RX_BUF[1], g_cyRevBufffLen - 3) )
    {
    	//发送数据错误
    	return 1;        
    }

    cyLrc = MODBUS_ASCII_GetLrc(&USART_RX_BUF[1], g_cyRevBufffLen - 5);
    if (cyLrc != MODBUS_ASCII_AsciiToHex(&USART_RX_BUF[g_cyRevBufffLen - 4]) )
    {
    	//发送数据效验错误
    	eres = 2;
    }
    ...
    *pCyLen = MODBUS_ASCII_AsciiPacketToRtuPacket(&USART_RX_BUF[1], g_cyRevBufffLen - 5, cyRecvBuff);
...
//    return (UART1_SendData(cyAsciiBuff, cyAsciiLen) );
	return (HAL_UART_Transmit(&UART1_Handler, cyAsciiBuff, cyAsciiLen, 1000));
}
