   ...
   // SendBuf[SendLen++] = Register_Num * 2;
              ...
              || ((USART_RX_BUF[4] == 0x00) && (USART_RX_BUF[5] == 0x00))))
    {
      ...
      return;
        ...
        {
            case 0x50:   //标定低液位/高液位	
             if((USART_RX_BUF[4] == 0x00) && (USART_RX_BUF[5] == 0x00))//标定低液位
             {
             ...
             else if((USART_RX_BUF[4] == 0xFF) && (USART_RX_BUF[5] == 0x00))//标定高液位
              {
            ...
            case 0x51:	
               if ((USART_RX_BUF[4] == 0x00) && (USART_RX_BUF[5] == 0x00))
                  {
                    Eeprom_WriteByte(RUN_ADDR_BASE, "\x02", 1);  //恢复出厂设置
                    ReadPara(); 
...
void MBASC_Fun2B(void)
{
    ...
    if(UserPara.SlaveAddr != USART_RX_BUF[2])                                   //当从机地址改为非0x45时，也可以读写寄存器
    {
    ...
    Object_Num = USART_RX_BUF[4] * 256 + USART_RX_BUF[5];                       //读对象个数   
...
void MBASC_Function(void)
{
    ...
    {
        if(2 == MODBUS_ASCII_RecvData(USART_RX_BUF, &RecvLen))//校验错误
        {
            return;
        }  
        if(RecvLen <= 0)
